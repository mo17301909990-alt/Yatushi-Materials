<template>
  <div class="min-h-screen bg-gray-100 pt-20 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
      <!-- 页面标题 -->
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">水油匹配</h1>
        <p class="mt-1 text-sm text-gray-500">搜索水油(哑光)产品，按后加工工序筛选兼容性</p>
      </div>
      ...
      >
        <el-table :data="pagedResult.items" stripe style="width: 100%">
          ...
          <el-table-column label="厚度" min-width="100">
            <template #default="{ row }">
              {{ row.thickness ?? '-' }}
            </template>
          </el-table-column>
...
import { waterVarnishMatteApi, type WaterVarnishMatteProduct, type PagedItems } from '@/api/modules/waterVarnishMatte';
...
// 结果
const pagedResult = ref<PagedItems<WaterVarnishMatteProduct> | null>(null);
...
async function loadSteps() {
  try {
    const res: any = await waterVarnishMatteApi.getSteps();
    steps.value = res || [];
  } catch {
    // 静默失败，下拉为空
  }
...
async function loadMatch() {
  ...
  try {
    const res: any = await waterVarnishMatteApi.match({
...
/** 批量加载兼容性数据 */
async function loadCompatibilities(items: WaterVarnishMatteProduct[]) {
  const promises = items.map(async (product) => {
    if (!product.id) return;
    try {
      const detail: any = await waterVarnishMatteApi.getProductDetail(product.id);
