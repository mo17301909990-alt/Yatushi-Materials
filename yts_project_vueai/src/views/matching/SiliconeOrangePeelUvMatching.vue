<template>
  <div class="min-h-screen bg-gray-100 pt-20 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">硅胶桔纹UV</h1>
        <p class="mt-1 text-sm text-gray-500">搜索硅胶桔纹UV产品，按后加工工序筛选兼容性</p>
      </div>
...
import { siliconeOrangePeelUvApi, type SiliconeOrangePeelUvProduct, type PagedItems } from '@/api/modules/siliconeOrangePeelUv';
...
const pagedResult = ref<PagedItems<SiliconeOrangePeelUvProduct> | null>(null);
...
async function loadSteps() {
  try { const res: any = await siliconeOrangePeelUvApi.getSteps(); steps.value = res || []; } catch {}
}

async function loadMatch() {
  loading.value = true;
  try {
    const res: any = await siliconeOrangePeelUvApi.match({
...
async function toggleExpand(id: number) {
  ...
  try { const res: any = await siliconeOrangePeelUvApi.getProductDetail(id); currentDetail.value = res || null; }
