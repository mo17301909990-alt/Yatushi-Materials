<template>
  <div class="min-h-screen bg-gray-100 pt-20 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">LEO 非平面产品贴纸</h1>
        <p class="mt-1 text-sm text-gray-500">搜索LEO非平面产品贴纸产品，按后加工工序筛选兼容性</p>
      </div>
...
import { leoNonFlatApi, type LeoNonFlatProduct, type PagedItems } from '@/api/modules/leoNonFlat';
...
const pagedResult = ref<PagedItems<LeoNonFlatProduct> | null>(null);
...
async function loadSteps() {
  try { const res: any = await leoNonFlatApi.getSteps(); steps.value = res || []; } catch {}
}
...
async function loadMatch() {
  ...
  try {
    const res: any = await leoNonFlatApi.match({
...
async function loadCompatibilitiesForProducts(products: { id?: number }[]) {
  ...
  for (const product of products) {
    ...
    try {
      const res: any = await leoNonFlatApi.getCompatibilitiesByProductId(product.id);
      const comps: any[] = res || [];
...
async function toggleExpand(id: number) {
  ...
  try { const res: any = await leoNonFlatApi.getProductDetail(id); currentDetail.value = res || null; }
