<template>
  <div class="min-h-screen bg-gray-100 pt-20 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">硅胶水彩油墨</h1>
        <p class="mt-1 text-sm text-gray-500">搜索硅胶水彩油墨产品，按后加工工序筛选兼容性</p>
      </div>
...
import { siliconeWatercolorInkApi, type SiliconeWatercolorInkProduct, type PagedItems } from '@/api/modules/siliconeWatercolorInk';
...
const pagedResult = ref<PagedItems<SiliconeWatercolorInkProduct> | null>(null);
...
async function loadSteps() {
  try { const res: any = await siliconeWatercolorInkApi.getSteps(); steps.value = res || []; } catch {}
}

async function loadMatch() {
  loading.value = true;
  try {
    const res: any = await siliconeWatercolorInkApi.match({
...
async function toggleExpand(id: number) {
  ...
  try { const res: any = await siliconeWatercolorInkApi.getProductDetail(id); currentDetail.value = res || null; }
