<template>
  <div class="min-h-screen bg-gray-100 pt-20 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">硅胶水性透明白墨</h1>
        <p class="mt-1 text-sm text-gray-500">搜索硅胶水性透明白墨产品，按后加工工序筛选兼容性</p>
      </div>
...
import { siliconeWaterBaseTransparentApi, type SiliconeWaterBaseTransparentProduct, type PagedItems } from '@/api/modules/siliconeWaterBaseTransparent';
...
const pagedResult = ref<PagedItems<SiliconeWaterBaseTransparentProduct> | null>(null);
...
async function loadSteps() {
  try { const res: any = await siliconeWaterBaseTransparentApi.getSteps(); steps.value = res || []; } catch {}
}

async function loadMatch() {
  loading.value = true;
  try {
    const res: any = await siliconeWaterBaseTransparentApi.match({
...
async function toggleExpand(id: number) {
  ...
  try { const res: any = await siliconeWaterBaseTransparentApi.getProductDetail(id); currentDetail.value = res || null; }
