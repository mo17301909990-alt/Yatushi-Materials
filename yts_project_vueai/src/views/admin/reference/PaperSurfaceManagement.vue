<template>
  <div class="paper-surface-management">
    <div class="page-header">
      <div class="page-header-row">
        <AdminBackToPanel />
        <div class="page-header-text">
          <h1 class="page-title">紙張面字典管理</h1>
          <p class="page-description">管理紙張面分類字典（有粉紙/無粉紙/膠底紙/布料/塑膠/其它）</p>
        </div>
      </div>
    </div>
...
import { paperSurfaceApi } from '@/api/modules/paperSurface'
import type { PaperSurface } from '@/types/materialManagement'

// 数据
const loading = ref(false)
const saving = ref(false)
const dataList = ref<PaperSurface[]>([])
...
const formRef = ref()
const formData = ref<Partial<PaperSurface>>({
  category: '',
  detailName: '',
  displayOrder: 0
})
...
const formRules = {
  ...
  detailName: [
    { required: true, message: '請輸入詳細名稱', trigger: 'blur' },
...
const loadData = async () => {
  loading.value = true
  try {
    const response = await paperSurfaceApi.getAll()
...
// 编辑
const handleEdit = (row: PaperSurface) => {
...
// 删除
const handleDelete = async (row: PaperSurface) => {
  try {
    await ElMessageBox.confirm(
      `確定要刪除紙張面「${row.detailName}」嗎？`,
      '確認刪除',
      {
    ...
    await paperSurfaceApi.delete(row.id!)
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEditing.value && formData.value.id) {
      await paperSurfaceApi.update(formData.value.id, formData.value)
      ElMessage.success('更新成功')
    } else {
      await paperSurfaceApi.create(formData.value)
